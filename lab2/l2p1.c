...
int main()
{
  ...
  for (int bcount = 0; bcount < 8 && (fread(buff, byte, 1, file) == 1); bcount++)
  {
    //reads in ip addres
    if (bcount < 4)
    {
    ...
    if (bcount == 7)
    {
      char name[256];
      char absoulte = 'F';
      for (int i = 0; num_names != 0 && (fread(buff, byte, 1, file) == 1); i++)
      {
  ...
  //and extra malloc is done so i need to free what i does
  ...
  jrb_traverse(tmp_node, machines)
  {
    ...
    free_dllist(ip->names);
    //freed the stuff in ip now i delete ip
    free(ip);
  }
