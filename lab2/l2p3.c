...
int main()
{
  ...
  buff = (unsigned char *)malloc(350000);

  int buff_size = read(file_int, buff, 350000);
  ...
  for (int bcount = 0; bcount < 8 && (buff_size >= buff_index); bcount++)
  {
    //reads in ip addres
    if (bcount < 4)
    {
      ip->address[bcount] = (unsigned char)(buff[buff_index]);
    }

    //reads in the number of names
    if (bcount >= 4)
    {
      num_names |= ((unsigned int)(buff[buff_index])) << (8 * (7 - bcount));
    }
    ...
    if (bcount == 7)
    {
      ...
      //printf("%s %d\n",dll_first(ip->names)->val.s, buff_index);
      
      //extra increment at end of loop 
      bcount = -1;
      num_names = 0;
      ip = (IP *)malloc(sizeof(IP));
      ip->names = new_dllist();
  ...
  // mallocs and extra ip so free it 
  ...
  while (!feof(stdin))
  {
    ...
    Dllist p;
    char boo = 'F';
    //runs through the tree when it sees something the same as str it prints the info for that machine
